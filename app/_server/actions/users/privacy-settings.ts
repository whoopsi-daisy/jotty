...
const PRIVACY_SETTINGS_FILE = path.join(
  process.cwd(),
  "data",
  "users",
...
async function writePrivacySettings(
  settings: Record<string, PrivacySettings>
): Promise<void> {
  try {
    await fs.mkdir(path.dirname(PRIVACY_SETTINGS_FILE), { recursive: true });
    await fs.writeFile(
      PRIVACY_SETTINGS_FILE,
...
export async function updatePrivacySettingsAction(
  ...
  try {
    ...
    const allowSessionTracking =
      formData.get("allowSessionTracking") === "true";
    const dataRetentionDays =
      parseInt(formData.get("dataRetentionDays") as string) || 30;
