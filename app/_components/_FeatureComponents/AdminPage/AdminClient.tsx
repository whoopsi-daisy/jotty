...
import { Users, FileText, CheckSquare, Settings, Activity, Shield, Trash2, Edit3, Plus, Search, ArrowLeft } from "lucide-react";
import { Button } from "@/app/_components/UI/Elements/button";
import { UserManagementModal } from "@/app/_components/UI/Modals/UserManagementModal";
import { User, List, Document } from "@/app/_types";
...
export function AdminClient({ username }: AdminClientProps) {
    ...
    const [activeTab, setActiveTab] = useState<"overview" | "users" | "content" | "sharing">("overview");
    ...
    const loadAdminData = async () => {
        ...
        try {
            ...
            setSharedItems(sharedData.success && sharedData.data ? sharedData.data : {});
        } catch (error) {
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                            ...
                            <div className="text-right">
                                <p className="text-sm text-muted-foreground">Checklists: {stats.sharedChecklists}</p>
                                <p className="text-sm text-muted-foreground">Documents: {stats.sharedDocuments}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-border">
                            {filteredUsers.map((user) => {
                                const userChecklists = allLists.filter(list => list.owner === user.username).length;
                                const userDocs = allDocs.filter(doc => doc.owner === user.username).length;
                ...
                <div className="bg-background border border-border rounded-lg p-6">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                        {sharedItems.sharedWithMe?.checklists?.length > 0 ? (
                            ...
                            <p className="text-sm text-muted-foreground">No shared checklists</p>
                        )}
                    </div>
                </div>
