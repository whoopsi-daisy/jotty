...
export async function deleteAccountAction(formData: FormData): Promise<Result<null>> {
    try {
        ...
        const passwordHash = createHash("sha256").update(confirmPassword).digest("hex");
        ...
        try {
            ...
            const checklistsDir = path.join(process.cwd(), "data", "checklists", currentUser.username);
            await fs.rm(checklistsDir, { recursive: true, force: true });

            // Delete user's documents directory
            const docsDir = path.join(process.cwd(), "data", "docs", currentUser.username);
